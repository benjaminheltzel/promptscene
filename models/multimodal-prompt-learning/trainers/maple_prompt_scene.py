...
def load_clip_to_cpu(cfg):
    ...
    try:
        ...
        state_dict = torch.load(model_path, map_location="cpu")
    design_details = {"trainer": 'MaPLePromptScene',
                      "vision_depth": cfg.TRAINER.MAPLE_PROMPT_SCENE.PROMPT_DEPTH_VISION,
                      "language_depth": cfg.TRAINER.MAPLE_PROMPT_SCENE.PROMPT_DEPTH_TEXT, "vision_ctx": cfg.TRAINER.MAPLE_PROMPT_SCENE.N_CTX_VISION,
                      "language_ctx": cfg.TRAINER.MAPLE_PROMPT_SCENE.N_CTX_TEXT}
    model = clip.build_model(state_dict or model.state_dict(), design_details)
...
class VLPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        assert cfg.TRAINER.MAPLE_PROMPT_SCENE.PROMPT_DEPTH_TEXT >= 1, "In Independent VL prompting, Language prompt depth should be >=1" \
                                                        "\nPlease use VPT trainer if you want to learn only vision " \
                                                        "branch  "
        n_ctx = cfg.TRAINER.MAPLE_PROMPT_SCENE.N_CTX_TEXT
        ...
        print(f"Number of context words (tokens) for Vision prompting: {cfg.TRAINER.MAPLE_PROMPT_SCENE.N_CTX_VISION}")
        self.ctx = nn.Parameter(ctx_vectors)
